...
///      	  a DNS query for
/// @param[in,out] out A std::vector of std::string containing the
///			   the results of the DNS lookups
void MathildaUtils::name_to_addr_a(std::vector<std::string> const &hostnames, std::vector<std::string> &out) {
	...
	for(uint32_t proc_num = 0; proc_num <= num_cores; proc_num++) {
		...
		if(p == ERR) {
			...
			for(it = hostnames.begin()+start; it != hostnames.begin()+end; ++it) {
				...
				shm_store_string(mf->my_proc_info.shm_ptr, o.c_str(), o.size());
			}
...
///
/// @param[in] shm_head Pointer to start of shared memory
/// @param[in] str Pointer to a C string
/// @param[in] sz Size of the string
int MathildaUtils::shm_store_string(uint8_t *shm_head, const char *str, size_t sz) {
	RET_ERR_IF_PTR_INVALID(shm_head);
	...
	StringEntry *tmp = (StringEntry *) shm_head;
	uint8_t *string = NULL;
	...
	if((tmp - (StringEntry *) shm_head) > SHM_SIZE) {
		return ERR;
	}
